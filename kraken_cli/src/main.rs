...
fn test_get_tradable_assets(rest_client: &RestClient) -> RestResult<()> {
    let assets = rest_client.get_tradable_asset_pairs()?;
    println!("Tradable Assets: {:?}", assets);
    Ok(())
}

fn test_get_ticker_info(rest_client: &RestClient) -> RestResult<()> {
    let ticker_info = rest_client.get_ticker_information(&["XBTUSD", "ETHUSD"])?;
